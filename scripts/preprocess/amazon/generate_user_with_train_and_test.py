...
for index, user in enumerate(all_users):
    print(f'checking user{user} {index}/{len(all_users)}')
    ...
    if len(train_data) != 0:
        ...
        if len(test_data) != 0:
            ...
            user_with_train_and_test.append(user)
...
# 将结果保存到json文件中
save_path = os.path.join(path_util.get_amazon_preprocess_fp(), 'user_with_train.json')
with open(save_path, 'w') as f:
    json.dump(user_with_train, f)
print(f"user with train data successfully saved to {save_path}")

save_path = os.path.join(path_util.get_amazon_preprocess_fp(), 'user_with_train_and_test.json')
with open(save_path, 'w') as f:
