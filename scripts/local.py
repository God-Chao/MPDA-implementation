...
def main():
    ...
    for index, user in enumerate(test_users):
        ...
        local_auc = train_util.test_model_with_dataset(init_model, test_loader, device)
        local_list.append(local_auc)
        print(f'Local user{user} = {local_auc}')

    # 保存数据
    run_name = dataset_name +'_'+ model_name + '_Local'
    ...
    names = ['user_id', 'num_train_samples', 'num_test_samples', 'Local']
    # 创建一个工作簿和工作表
    wb = Workbook()
    ws = wb.active
    ...
    # 将数据写入表格
    for user_id, train_samples, test_samples, local in zip(user_id_list, num_train_samples_list, num_test_samples_list, local_list):
        ws.append([user_id, train_samples, test_samples, local])
